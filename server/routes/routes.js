...
router.post("/saveCategory", async (req, res) => {
  try {
    const { name, user_id, color, max } = req.body;
    const query = `
      INSERT INTO categories (name, user_id, color, max)
      VALUES ($1, $2, $3, $4)
      RETURNING *;`;

    const values = [name, user_id, color, max];
    db.query(query, values, (err, result) => {
      if (err) {
...
router.patch("/updateCategory", async (req, res) => {
  try {
    const { id, name, color, max } = req.body;
    const query = `
      UPDATE categories
      SET name = $1, color = $2, max = $4
      WHERE id = $3
      RETURNING *;`;

    const values = [name, color, id, max];
    const result = await db.query(query, values);
    res.json(result.rows[0]);
  } catch (error) {
