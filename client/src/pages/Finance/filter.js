  ...
  return (
    ...
    >
      <Box sx={{ display: "flex", alignItems: "center", gap: 1, mr: 1 }}>
        <FilterListIcon sx={{ fontSize: 18, color: isDark ? "rgba(198,170,96,0.7)" : theme.palette.primary.main }} />
        <Typography variant="caption" sx={{ fontWeight: 700, color: isDark ? "rgba(198,170,96,0.7)" : theme.palette.primary.main, letterSpacing: "0.06em", textTransform: "uppercase" }}>
          Filter
        </Typography>
      </Box>

      <FormControl size="small" sx={{ minWidth: 130 }}>
        <Select value={filterMonth} onChange={(e) => setFilterMonth(e.target.value)} sx={selectSx} displayEmpty>
          {months.map((m) => (
      ...
      <FormControl size="small" sx={{ minWidth: 150 }}>
        ...
        >
          <MenuItem value=""><Typography variant="body2" sx={{ opacity: 0.5 }}>Alle Kategorien</Typography></MenuItem>
          {categories.map((c) => (
        ...
        InputProps={{
          startAdornment: (
            <InputAdornment position="start">
              <SearchIcon sx={{ fontSize: 17, color: isDark ? "rgba(255,255,255,0.35)" : "rgba(0,0,0,0.35)" }} />
            </InputAdornment>
          ),
        }}
        sx={{
          ...
          "& .MuiOutlinedInput-root": {
            ...
            "&:hover fieldset": { borderColor: isDark ? "rgba(198,170,96,0.5)" : "rgba(78,87,123,0.4)" },
